...
{	
	...
	max_valـ = max_val_from;

	//create lut
	shared_ptr<uint16_t> temp = shared_ptr<uint16_t>(new uint16_t[max_val_from+1], [](uint16_t *p) { delete[] p; });
	lookup_tableـ = temp;
	lookup_table_ptrـ = temp.get();

	//fill lut
	for(int i=0; i<max_val_from; i++) {
		lookup_table_ptrـ[i] = min(max((int)round((pow((((i + offset) * gain) / max_val_from), gamma) * max_val_to)), 0), max_val_to);
	}
}
...
{
	#ifdef DEBUG
		assert_util(from <= max_valـ, "pixel_value-from bigger than max expected value.");
	#endif
	lookup_table_ptrـ[from] = to;
}

uint16_t BD_ConverterGGO::convert(uint16_t pixel_value)
{
	#ifdef DEBUG
		assert_util(pixel_value <= max_valـ, "pixel_value bigger than max expected value.");
	#endif
	return lookup_table_ptrـ[pixel_value];
}
